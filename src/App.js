...
function App() {
  ...
  var [background, setBackground] = useState("app");
  ...
  const searchLocation = (event) => {
    if (event.key === "Enter") {
      axios.get(weatherAPI).then((response) => {
        ...
        if (5 < hours && hours < 12) {
          console.log("morning");
          setBackground("appmorning");
        } else if (11 < hours && hours < 18) {
          console.log("mid-day");
          setBackground("./assets/mid-day.jpg");
        } else if ((17 < hours && hours < 23) || (-1 < hours && hours < 6)) {
          console.log("night");
          setBackground("appevning");
        }
      });
    }
  };
      ...
      <div className="container">
        <div className="top">
          <div className="location">
            <p>
              {data.name
                ? [data.name, ", ", data.sys.country, ", ", currTime]
                : null}
            </p>
            {flagURL ? (
              <img
